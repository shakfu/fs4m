...
void fs_dsp64(t_fs* x, t_object* dsp64, short* count, double samplerate, long maxvectorsize,
              long flags)
{
    ...
    if (x->out_maxsize < maxvectorsize) {
        ...
        x->left_buffer = (float*)sysmem_resizeptrclear(x->left_buffer,
            sizeof(float) * maxvectorsize);
        x->right_buffer = (float*)sysmem_resizeptrclear(x->right_buffer,
            sizeof(float) * maxvectorsize);
...
void fs_perform64(t_fs* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts,
...
void fs_do_load(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    if (argc > 0 && is_symbol(argv)) {
        ...
        if (sf == NULL) {
            ...
            fs_error(x, "soundfont named '%s' is already loaded", name->s_name);
        }
    }
}
    ...
    if (is_number(argv)) {
        ...
        if (sf != NULL) {
            ...
            if (fluid_synth_sfunload(x->synth, id, 0) >= 0) {
                fs_post(x, "unloaded soundfont '%s' (id %d)", name->s_name, id);
                return;
            }
        }
        ...
        if (sym == gensym("all")) {
            ...
            while (sf != NULL) {
                ...
                if (fluid_synth_sfunload(x->synth, id, 0) >= 0) {
                    ...
                    fs_error(x, "cannot unload soundfont '%s' (id %d)", name->s_name, id);
                }
            ...
            if (sf != NULL) {
                ...
                if (fluid_synth_sfunload(x->synth, id, 0) >= 0) {
                    fs_post(x, "unloaded soundfont '%s' (id %d)", sym->s_name, id);
                    return;
                }
            }
        ...
        if (is_number(argv)) {
            ...
            if (sym == gensym("all")) {
                ...
                for (i = 0; i < n; i++) {
                    ...
                    if (fluid_synth_sfreload(x->synth, id) >= 0) {
                        fs_post(x, "reloaded soundfont '%s' (id %d)", name->s_name, id);
                    } else {
                        fs_error(x, "cannot reload soundfont '%s' (id %d)", name->s_name, id);
                    }
                }
            } else {
                fluid_sfont_t* sf = fs_sfont_get_by_name(x, sym);
                ...
                if (sf != NULL) {
                    ...
                    if (fluid_synth_sfreload(x->synth, id) >= 0) {
                        fs_post(x, "reloaded soundfont '%s' (id %d)", sym->s_name, id);
                        return;
                    }
                }
...
void fs_note(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    if (argc > 0 && is_number(argv)) {
        ...
        case 1:
            fluid_synth_noteon(x->synth, channel - 1, get_number_as_int(argv), velocity);
        case 0:
            break;
        }
    }
...
void fs_list(t_fs* x, t_symbol* s, short argc, t_atom* argv) { fs_note(x, NULL, argc, argv); }
...
void fs_control_change(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    if (argc > 0 && is_number(argv)) {
        ...
        case 1:
            fluid_synth_cc(x->synth, channel - 1, get_number_as_int(argv), value);
        case 0:
            break;
        }
    }
...
void fs_pitch_bend_wheel(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    if (argc > 0 && is_number(argv)) {
        ...
        fluid_synth_pitch_wheel_sens(x->synth, channel - 1, get_number_as_int(argv));
    }
}
...
void fs_program_change(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    if (argc > 0 && is_number(argv)) {
        ...
        fluid_synth_program_change(x->synth, channel - 1, get_number_as_int(argv));
    }
}
...
void fs_bank_select(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    if (argc > 0 && is_number(argv)) {
        ...
        fluid_synth_bank_select(x->synth, channel - 1, get_number_as_int(argv));
        fluid_synth_get_program(x->synth, channel - 1, &sf_id, &bank_num, &prog_num);
...
void fs_select(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    ...
    case 1:
        if (is_number(argv)) {
            fluid_synth_program_select(x->synth, channel - 1, get_number_as_int(argv), bank,
                                       preset);
        } else if (is_symbol(argv)) {
            t_symbol* name = atom_getsym(argv);
            fluid_sfont_t* sf = fs_sfont_get_by_name(x, name);
            ...
            if (sf != NULL) {
                fluid_synth_program_select(x->synth, channel - 1, fluid_sfont_get_id(sf), bank,
                                           preset);
            } else
...
{
    post("fluidsynth~, version %s (based on FluidSynth %s)", FS_VERSION, FLUIDSYNTH_VERSION);
        ...
        if (argc > 1) {
            ...
            if (sf != NULL) {
                ...
                while ((preset = fluid_sfont_iteration_next(sf)) != NULL) {
                    ...
                    fs_post(x, "  '%s': bank %d, program %d", preset_name->s_name,
                        bank_num, prog_num);
                }
            }
        } else {
...
void fs_print_channels(t_fs* x, t_symbol* s, short argc, t_atom* argv)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (preset != NULL) {
            ...
            fs_post(x, "  %d: soundfont '%s', bank %d, program %d: '%s'", i + 1,
                    fs_sfont_get_name(sf)->s_name, bank_num, prog_num, preset_name->s_name);
        } else
