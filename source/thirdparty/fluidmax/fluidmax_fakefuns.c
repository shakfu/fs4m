 ...
 */
/*
 *  This file contains the (mostly empty) implementation of some functions
 * without which Fluidsynth wouldn't compile.
 *
 *  The Max/MSP version of FluidSynth tries to link only with a minimum of
 * needed files. Some of the linked files depend on functions that are not
 * necessary and that are implemnted in files we don't want to include. So,
 * alternate or empty implemntations of these functions are provided here.
 *
 */
...
unsigned int fluid_curtime() { return (unsigned int)gettime(); }

double fluid_utime(void) { return 0.0; }
...
char* fluid_error() { return NULL; }

/* This code is (unelegantly) copied from fluid_sys.c since other parts of
 * fluid_sys.c that we don't need here depend on many other things that we
 * don't want here. */
char* fluid_strtok(char** str, char* delim)
{
    ...
    for (s = s + 1; *s; s++) {
        ...
        for (d = delim; *d; d++) /* is source char a token char? */
        {
            if (c == *d) /* token char match? */
            {
                *s = '\0'; /* overwrite token char with zero byte to terminate
                              token */
                *str = s
                    + 1; /* update str to point to beginning of next token */
                return token;
            }
        }
    }
...
int fluid_midi_event_get_type(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_channel(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_key(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_velocity(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_control(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_value(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_program(fluid_midi_event_t* evt) { return 0; }
...
int fluid_midi_event_get_pitch(fluid_midi_event_t* evt) { return 0; }
