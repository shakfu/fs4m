 ...
 *    (8): added message resample permitting to chose resampling interpolation
 * method (7): added names for soundfonts (== file name without path and
 * postfix) (6): added message 'info' (5): fixed bogus path translation at file
 * loading
 *
 */
...
static fluid_sfont_t* fluidmax_sfont_get_by_name(fluidmax_t* self,
                                                 ftmax_symbol_t name)
{
    ...
    if (ac > 0 && ftmax_is_symbol(at)) {
        ...
        if (sf == NULL) {
            ...
            error("fluidsynth~: soundfont named '%s' is already loaded",
                  ftmax_symbol_name(name));
            return;
        }
    }
}

static void fluidmax_load_with_dialog(t_object* o, t_symbol* s, short argc,
                                      t_atom* argv)
{
    char filename[256];
    char maxpath[1024];
    ...
    if (path_topotentialname(path, filename, maxpath, 0) == 0) {
        ...
        ftmax_set_symbol(
            &a,
            ftmax_new_symbol(fluidmax_translate_fullpath(maxpath, fullpath)));
    ...
    if (ac == 0)
        ...
        if (ftmax_is_symbol(at)) {
            ...
            if (string[0] == '/')
                ...
                ftmax_set_symbol(&a,
                                 ftmax_new_symbol(fluidmax_translate_fullpath(
                                     maxpath, fullpath)));
        ...
        if (ftmax_is_number(at)) {
            ...
            if (sym == ftmax_new_symbol("all")) {
                ...
                if (sf != NULL) {
                    ...
                    if (fluid_synth_sfunload(self->synth, id, 0) >= 0) {
                        post("fluidsynth~: unloaded soundfont '%s' (id %d)",
                             ftmax_symbol_name(sym), id);
                        return;
                    }
                }

                error("fluidsynth~: cannot unload soundfont '%s'",
                      ftmax_symbol_name(sym));
            }
        }
    }
}
...
static void fluidmax_note(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    if (ac > 0 && ftmax_is_number(at)) {
            ...
            if (ftmax_is_number(at + 2)) {
                ...
                if (channel < 1)
                    channel = 1;
                         ...
                         > fluid_synth_count_midi_channels(self->synth))
        ...
        case 1:
            fluid_synth_noteon(self->synth, channel - 1,
                               ftmax_get_number_int(at), velocity);
        case 0:
            break;
        }
    }
...
static void fluidmax_control_change(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    if (ac > 0 && ftmax_is_number(at)) {
        ...
        case 1:
            fluid_synth_cc(self->synth, channel - 1, ftmax_get_number_int(at),
                           value);
        case 0:
            break;
        }
...
static void fluidmax_pitch_bend_wheel(t_object* o, Symbol* s, short ac,
                                      Atom* at)
{
    fluidmax_t* self = (fluidmax_t*)o;
    ...
    if (ac > 0 && ftmax_is_number(at)) {
        ...
        fluid_synth_pitch_wheel_sens(self->synth, channel - 1,
                                     ftmax_get_number_int(at));
    }
}
...
static void fluidmax_program_change(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    if (ac > 0 && ftmax_is_number(at)) {
        ...
        fluid_synth_program_change(self->synth, channel - 1,
                                   ftmax_get_number_int(at));
    }
}
...
static void fluidmax_bank_select(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    if (ac > 0 && ftmax_is_number(at)) {
        ...
        fluid_synth_bank_select(self->synth, channel - 1,
                                ftmax_get_number_int(at));
        fluid_synth_get_program(self->synth, channel - 1, &sf_id, &bank_num,
                                &prog_num);
...
static void fluidmax_select(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    case 1:
        if (ftmax_is_number(at))
            fluid_synth_program_select(self->synth, channel - 1,
                                       ftmax_get_number_int(at), bank, preset);
        else if (ftmax_is_symbol(at)) {
            ftmax_symbol_t name = ftmax_get_symbol(at);
            fluid_sfont_t* sf = fluidmax_sfont_get_by_name(self, name);
...
static void fluidmax_reverb(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    if (ac == 0) {
        ...
        case 1:
            fluid_synth_set_reverb(self->synth, room, damping, width,
                                   ftmax_get_number_float(at));
        case 0:
            break;
        }
    } else if (ftmax_is_symbol(at)) {
...
static void fluidmax_chorus(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    if (ac == 0) {
        ...
        case 1:
            fluid_synth_set_chorus(self->synth, nr, ftmax_get_number_float(at),
                                   speed, depth, type);
        case 0:
            break;
        }
...
static void fluidmax_set_resampling_method(t_object* o, Symbol* s, short ac,
                                           Atom* at)
{
    fluidmax_t* self = (fluidmax_t*)o;
    ...
    if (ac > 0) {
        if (ftmax_is_number(at)) {
            ...
            if (ip == 0)
                fluid_synth_set_interp_method(self->synth, -1,
                                              FLUID_INTERP_NONE);
            else if (ip < 3)
                fluid_synth_set_interp_method(self->synth, -1,
                                              FLUID_INTERP_LINEAR);
...
static void fluidmax_tuning_octave(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    fluid_synth_create_octave_tuning(self->synth, tuning_bank, tuning_prog,
                                     ftmax_symbol_name(name), pitch);
}
...
static void fluidmax_tuning_select(t_object* o, Symbol* s, short ac, Atom* at)
{
    ...
    fluid_synth_select_tuning(self->synth, channel - 1, tuning_bank,
                              tuning_prog);
}
            ...
            if (sym == sym_soundfonts) {
                        ...
                        if (sf != NULL) {
                            ...
                            while (fluid_sfont_iteration_next(sf, &preset)
                                ...
                                char* preset_str = fluid_preset_get_name(
                                    &preset);
                                ftmax_symbol_t preset_name = ftmax_new_symbol(
                                    preset_str);
                                ...
                                post("  '%s': bank %d, program %d",
                                     ftmax_symbol_name(preset_name), bank_num,
                                     prog_num);
                            }
                        }
                    } else {
                        ...
                        for (i = 0; i < 128; i++) {
                            ...
                            for (j = 0; j < 128; j++) {
                                ...
                                if (preset != NULL) {
                                    ...
                                    char* preset_str = fluid_preset_get_name(
                                        preset);
                                    ftmax_symbol_t preset_name
                                        = ftmax_new_symbol(preset_str);
                ...
                for (i = 0; i < n; i++) {
                    ...
                    if (preset != NULL) {
                        ...
                        fluid_synth_get_program(self->synth, i, &sf_id,
                                                &bank_num, &prog_num);
                ...
                for (i = 0; i < n; i++) {
                    ...
                    double incr = fluid_synth_get_gen(self->synth, channel - 1,
                                                      i);
                ...
                if (self->chorus != 0) {
                    ...
                    double speed = fluid_synth_get_chorus_speed_Hz(
                        self->synth);
                    double depth = fluid_synth_get_chorus_depth_ms(
                        self->synth);
                    int type = fluid_synth_get_chorus_type(self->synth);
                    int nr = fluid_synth_get_chorus_nr(self->synth);
                    ...
                    if (ac > 1) {
                        ...
                        if (sf != NULL) {
                            ...
                            while (fluid_sfont_iteration_next(sf, &preset)
                                ...
                                outlet_anything(self->outlet, sym_preset, 4,
                                                a);
                            }
                        }
                    } else {
                        ...
                        for (i = 0; i < 128; i++) {
                            ...
                            for (j = 0; j < 128; j++) {
                                ...
                                if (preset != NULL) {
                                    ...
                                    ftmax_set_int(a + 2, i);
                                    ftmax_set_int(a + 3, j);
                                    outlet_anything(self->outlet, sym_preset,
                                                    4, a);
                                }
                            }
                        }
...
static void* fluidmax_new(Symbol* s, short ac, Atom* at)
{
    ...
    if (self->settings != NULL) {
        fluid_settings_setint(self->settings, "synth.midi-channels",
                              midi_channels);
...
int main(void)
{
    setup(&fluidmax_class, (method)fluidmax_new, (method)fluidmax_free,
          (short)sizeof(fluidmax_t), 0, A_GIMME, 0);
    dsp_initclass();
